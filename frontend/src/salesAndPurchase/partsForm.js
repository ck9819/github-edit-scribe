...
import { fetchItems, fetchItemId } from '../store/slices//itemSlice';
import {addItems} from '../store/api/index';
...
const PartForm = () => {
  ...
  const { loading, items,itemId, error } = useSelector((state) => state.itemData);

  useEffect(() => {
    dispatch(fetchItems());
    dispatch(fetchItemId());
  }, [dispatch]);
  ...
  const onFinish = async (values) => {
      ...
      const response = dispatch(addItems(itemDetails));
      form.resetFields()
  }
...
console.log("items in parts",items);
  return (
    <>
